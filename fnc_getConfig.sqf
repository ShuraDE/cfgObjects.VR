...
_cfg = "(
    ...
    {getNumber (_x >> 'side') == 1 &&
      {getText (_x >> 'vehicleClass') in  'Fortress','Building','Stratetic','Thing']}
    }
...
["[_class,_genMac,_type,_description,_roles,_weapons,_magazines,_type2,_filter,_side,_model,_parent,_vehicleClass,_ttl]"] call ADL_DEBUG;
["[_cargoCoDriver,_transportSoldier,_transportVehicle,_transportAmmo,_transportFuel,_transportRepair,_maximumLoad,_transportMaxMagazines,_transportMaxWeapons,_transportMaxBackpacks]"] call ADL_DEBUG;
["[_fuelCap,_armour,_audible,_accuracy,_camouflage,_accerleration,_breakDist,_maxSpeed,_minSpeed,_hiddenSel]"] call ADL_DEBUG;
...
for[{_i = 1}, {_i < count(_cfg)}, {_i=_i+1}] do
{
        ...
        ["data idx: " + str(_i)]  call ADL_DEBUG;
             ...
             if (!isNil("_veh") && (typeName _veh == "OBJECT") && (!(_veh isKindOf "Logic")) && (alive _veh)) then {
               ...
               try {
                 ...
                 if (ENABLE_2ND_VEH_TD) then {
                   ...
                   _vehTD setDir 180;
                   _vehTD setVectorDirAndUp [[0,0.6,0],[0.6,0,0]];
                 };

                 //take screen shoot if enabled
                 if (ENABLE_SCREEN) then {
                   sleep 0.5;
                   _retVal = [_class] call FNC_SCR_CAP;
                   ["**** SCREEN " + _retVal + "****"] call ADL_DEBUG;
                   _scr = true;
                 };
